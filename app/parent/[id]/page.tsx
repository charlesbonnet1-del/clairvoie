...
export default async function ParentTicketPage({
  ...
  const ticket = await prisma.ticket.findFirst({
    where: { id: params.id, parentPseudoId: identity.pseudoId },
  ...
  const peutCloturer =
    ticket.gravite !== "grave" && ["ouvert", "attente_cloture_parent"].includes(ticket.statut);
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      <Link href="/parent" className="text-sm text-clairvoie-bleuclair underline">
      ...
      <div className="card space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">
              {ticket.etablissement.nom} · déposé le {ticket.createdAt.toLocaleDateString("fr-FR")}
            </p>
          </div>
        ...
        {ticket.statut === "attente_cloture_parent" && (
          <p className="text-xs text-teal-700">
            L&apos;établissement n&apos;a pas contesté ce signalement. Confirmez la clôture
            ci-dessous, sans quoi il sera transmis automatiquement au rectorat après{" "}
            {DELAI_CLOTURE_PARENT_JOURS} jours sans action de votre part.
          </p>
        )}
        ...
        {ticket.gravite === "grave" &&
          ["ouvert", "triangulation_requise", "escaladé", "escaladé_rectorat"].includes(
            ...
            <p className="text-xs text-amber-700">
              Signalement classé grave : la clôture ne peut se faire que par validation de
              l&apos;association tierce, pas par accord mutuel direct.
            </p>
          )}
        ...
        <div className="border-t border-slate-100 pt-3">
          {plainteDirecteDeclaree ? (
            <p className="text-xs text-slate-500">
              Vous avez indiqué avoir déposé plainte directement auprès de la police/gendarmerie
